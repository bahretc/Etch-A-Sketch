...
- **Off-LRS rows at the screen (strip studies).** A fiche row on the study route whose Milepost Road is a different linear reference (US 311 mileposted on "I 74 WB COUPLET", the concurrent freeway section) cannot fall in the route's milepost window, so `fiche-workbook --off-lrs-nis` screens it NIS without a review. This is the road the milepost is measured on, not the coded distance, which is never used to skip a review. Off by default; the engineer turns it on per study and confirms the rule (2026-09, 417 such rows on 260307016EA).
- **Wide-net intersection screen, two more no-review calls (tool, 2026-09, 05-20-62123).** A row coded in another municipality (`muni_ok`: the junction's town plus "0", rural; Creedmoor's Main Street on an Oxford fiche) is NIS before any placement. A row On a street that is no leg of the junction, measured between two other named streets that are no legs either, is NIS because none of the three roads reaches the junction; a DetailedFiche coordinate, when the row has one, outranks the names, and a blank Toward leaves the row unresolved. Both use the roads the fiche names, never the coded distance. `muni_ok` is off unless the engineer sets it per study.
