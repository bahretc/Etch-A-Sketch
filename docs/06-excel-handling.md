...
6. **External links to stale copies of the workbook are removed, not kept.** An office copy can carry formulas, drop-down lists and defined names that read `[n]Sheet!` from another copy of the same workbook (a Downloads path), so the numbers shown belong to another project and Excel raises the external-links warning. `drop_external_links` re-homes every such reference to the workbook's own sheet (defined names on sheets that do not exist here get an explicit new definition), renumbers the links that stay, and removes the link parts; `verify_integrity(allow_removed=...)` names exactly those parts. Links to real outside files (the NCDOT CRF sheet on the staff email sheet) stay.
