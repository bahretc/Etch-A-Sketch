...
def render_markdown(result: EvaluationResult, cfg: Config) -> str:
    ...
    lines.append(f"- **Route:** {a.route}  (MP {a.section_begin_mp}–{a.section_end_mp})")
    lines.append(f"- **Countermeasure:** {a.countermeasure}")
    lines.append(f"- **Study window:** {a.study_start} → {a.study_end}")
    ...
    if ba:
        ...
        lines.append(
            f"- **Total crashes:** {t['before']} → {t['after']} "
